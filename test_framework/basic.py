...
def gcc_compile_and_run(
    source_files: List[Path], options: List[str], target
    ...
    gcc = "gcc" if target == "x86_64" else "riscv64-linux-gnu-gcc"
    ...
    # run it
    qemu = [] if target == "x86_64" else ["qemu-riscv64", "-L", "/usr/riscv64-linux-gnu"]
    return subprocess.run(
        qemu + [exe], check=False, text=True, capture_output=True, timeout=10.0
    )
...
class TestChapter(unittest.TestCase):
    ...
    * compile_with_asm_lib_and_run:
    ...
    # the target architecture of the assembly generated by the compiler under test.
    # can either be x86_64 or riscv64
    target: str
    ...
    def invoke_compiler(
        ...
        target = [] if self.target == "x86_64" else ["--riscv64"]
        args = [self.cc] + self.options + target 
    ...
    def compile_and_run(self, source_file: Path) -> None:
        ...
        qemu = [] if self.target == "x86_64" else ["qemu-riscv64", "-L", "/usr/riscv64-linux-gnu"]
        result = subprocess.run(
            qemu + [exe], check=False, capture_output=True, text=True, timeout=10.0
        )
    ...
    def library_test_helper(
        ...
        if needs_mathlib(file_under_test) or any(needs_mathlib(f) for f in other_files):
            options.append("-lm")
        result = gcc_compile_and_run(source_files, options, self.target)
